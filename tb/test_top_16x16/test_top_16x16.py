...
async def load_w_full(dut, W, w_base):
    ...
    for n_tile in range(N_TILES):
        for k_tile in range(K_TILES):
            slab = W[k_tile * ROWS : (k_tile + 1) * ROWS, n_tile * COLS : (n_tile + 1) * COLS]
            dut.bd_w_we.value = 1
...
async def load_pch_full(dut, mults, shifts, mult_base, shift_base):
    N_TILES = len(mults) // COLS
    for n_tile in range(N_TILES):
        m_slab = mults[n_tile * COLS : (n_tile + 1) * COLS]
        s_slab = shifts[n_tile * COLS : (n_tile + 1) * COLS]
        await load_w_word(dut, mult_base + n_tile, pack_w_mults(m_slab))
        await load_w_word(dut, shift_base + n_tile, pack_w_shifts(s_slab))
...
async def wait_done(dut, max_cycles=3000):
...
async def configure(
    dut, *, M, N, K, ifm_base, w_base, ofm_base, flags=0, req_mult=1, req_shift=0, req_mult_base=0, req_shift_base=0, bias_base=0
):
...
@cocotb.test()
async def test_top_16x16_id(dut):
    cocotb.start_soon(Clock(dut.pclk, CLK_NS, units="ns").start())
    await reset(dut)
    assert await apb_read(dut, A_ID) == 0x4E50_5500


@cocotb.test()
async def test_top_16x16_gemm_only(dut):
    cocotb.start_soon(Clock(dut.pclk, CLK_NS, units="ns").start())
    await reset(dut)
    rng = np.random.default_rng(0x16_01)
    M, N, K = 4, 16, 16
    ...
    await configure(dut, M=M, N=N, K=K, ifm_base=0, w_base=0, ofm_base=0x40, flags=0, req_mult=1, req_shift=0)
    await kick(dut)
    await wait_done(dut)
    acc = gemm_i8(A, W)
...
@cocotb.test()
async def test_top_16x16_full_n32k32(dut):
    cocotb.start_soon(Clock(dut.pclk, CLK_NS, units="ns").start())
    await reset(dut)
    rng = np.random.default_rng(0x16_02)
    M, N, K = 8, 32, 32
    A = rng.integers(-8, 8, size=(M, K), dtype=np.int8)
    W = rng.integers(-8, 8, size=(K, N), dtype=np.int8)
    ...
    acc = model_bias_relu(acc, np.broadcast_to(bias_vec, acc.shape), bias_en=True, relu_en=True)
...
@cocotb.test()
async def test_top_16x16_per_channel(dut):
    cocotb.start_soon(Clock(dut.pclk, CLK_NS, units="ns").start())
    await reset(dut)
    rng = np.random.default_rng(0x16_03)
    M, N, K = 4, 32, 16
    A = rng.integers(-16, 16, size=(M, K), dtype=np.int8)
    W = rng.integers(-16, 16, size=(K, N), dtype=np.int8)
    mults = [int(1 << (16 + (i % 5))) for i in range(N)]
    shifts = [int(10 + (i % 6)) for i in range(N)]
    n_tiles = N // COLS
    k_tiles = K // ROWS
    w_base = 0
    ...
    await configure(
        ...
        req_mult=0,
        req_shift=0,
        req_mult_base=mult_base,
        req_shift_base=shift_base,
    )
    await kick(dut)
    await wait_done(dut)
    expected = model_req_pc(gemm_i8(A, W), mults, shifts)
    got = await read_ofm_full(dut, 0xA0, M, N)
    np.testing.assert_array_equal(got, expected)


@cocotb.test()
async def test_top_16x16_err_on_misaligned_dims(dut):
    cocotb.start_soon(Clock(dut.pclk, CLK_NS, units="ns").start())
    await reset(dut)
    await configure(dut, M=4, N=24, K=18, ifm_base=0, w_base=0, ofm_base=0x40, flags=0, req_mult=1, req_shift=0)
    await kick(dut)
    saw_err = 0
    for _ in range(8):
        await RisingEdge(dut.pclk)
